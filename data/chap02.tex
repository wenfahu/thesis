...
\begin{lstlisting}[caption={直接卷积实现},label={code:direct_conv}, mathescape=true]
  for n = 0 to N -1 do
    for k = 0 to K - 1 do
      for c = 0 to K -1 do
        for oj = 0 to P -1 do
...
\begin{lstlisting}[caption={优化直接卷积算法}, label={algo:opt_direct_conv}, breaklines, mathescape=true]
  $C_b$ = C / VLEN
  $K_b$ = K / VLEN
  $P_b = P / RB_P$
  $Q_b = Q / RB_Q$
...
\begin{lstlisting}[caption={im2col实现}, label={algo:im2col}]
...
\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{conv_sliding.png}
\caption{1D 卷积}
